...
class PythonNet(nn.Module):
    def __init__(self, channels: int):
        ...
        self.fc = nn.Sequential(
            nn.Flatten(),
            layer.Dropout(0.5),
            nn.Linear(channels * 8 * 8, 512, bias=False),
    ...
    class CextNet(nn.Module):
        def __init__(self, channels: int):
            ...
            self.fc = nn.Sequential(
                ...
                layer.SeqToANNContainer(nn.Linear(channels * 8 * 8, 512, bias=False)),
...
def main():
    ...
    * :ref:`中文API <classify_dvsg.__init__-cn>`
    ...
    '''
    parser = argparse.ArgumentParser(description='Classify CIFAR10DVS Gesture')
    parser.add_argument('-T', default=2, type=int, help='simulating time-steps')
    ...
    cache_dir=os.path.join('D:/下载/CIFAR10_DVS/cifar10dvs_cache',f'seed_{_seed_}_T_{args.T}')
