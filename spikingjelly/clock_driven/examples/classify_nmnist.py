...
class PythonNet(nn.Module):
    def __init__(self, channels: int):
        ...
        self.fc = nn.Sequential(
            nn.Flatten(),
            layer.Dropout(0.5),
            nn.Linear(128 * 8 * 8, 128 * 8 * 8//4, bias=False),
            #neuron.IFNode(v_threshold=1., v_reset=None,surrogate_function=surrogate.Sigmoid(),detach_reset=False),
            neuron.LIFNode(tau=2.0, surrogate_function=surrogate.ATan(), detach_reset=True),
            #nn.ReLU(),
            layer.Dropout(0.5),
            nn.Linear(128*8*8//4, 100, bias=False),
            neuron.LIFNode(tau=2.0, surrogate_function=surrogate.ATan(), detach_reset=True),
            #neuron.IFNode(v_threshold=1., v_reset=None, surrogate_function=surrogate.Sigmoid(), detach_reset=False)
            #nn.ReLU(),
        )
        ...
        def __init__(self, channels: int):
            ...
            self.fc = nn.Sequential(
                ...
                neuron.MultiStepLIFNode(tau=2.0, surrogate_function=surrogate.ATan(), detach_reset=True,
                                        backend='cupy'),
                layer.MultiStepDropout(0.5),
                layer.SeqToANNContainer(nn.Linear(128 * 4 * 4, 100, bias=False)),
    ...
    class CextNet2(nn.Module):
        def __init__(self, channels: int, T: int, b: int):
            ...
            self.conv2d = nn.Sequential(
                ...
                *CextNet2.block_2d(self, channels, channels),
                layer.MultiStepDropout(0.5),
                *CextNet2.block_2d(self, channels, 100),
                layer.MultiStepDropout(0.5),
                nn.Unflatten(0, (T, b))
            )
            self.vote = VotingLayer(10)
...
def main():
    ...
    * :ref:`中文API <classify_dvsg.__init__-cn>`
    ...
    train_set =NMNIST(args.data_dir, train=True, data_type='frame', split_by='number', frames_number=args.T)
    test_set = NMNIST(args.data_dir, train=False, data_type='frame', split_by='number', frames_number=args.T)
