...
class Sigmoid(SurrogateFunctionBase):
    ...
    def cuda_code(self, x: str, y: str, dtype='fp32'):
        ...
        # if dtype == 'fp32':
        #     code += f'''
        #     {tab4_str}const float {y} = ((1.0f - fabs({x}))>0.0f) * {x} * {alpha};
        #     '''
        #
        # elif dtype == 'fp16':
        #     code += f'''
        #     {tab4_str}const half2 {sg_name}_alpha = __float2half2_rn({alpha});
        #     {tab4_str}const half2 {y} = __hmul2(__hmul2(__hsub2(__float2half2_rn(1.0f), abs({x}))>0.0, {x}), {sg_name}_alpha);
        #     '''
        else:
            raise NotImplementedError
        code += f'''
...
class trangle(torch.autograd.Function):
    @staticmethod
    def forward(ctx, input, alpha, gama=1.0):
        out = (input > 0).float()
        L = torch.tensor([gama])
...
class Trangle(SurrogateFunctionBase):
    def __init__(self, alpha=2.0, spiking=True):
        '''
    ...
    @staticmethod
    def spiking_function(x, alpha):
        return trangle.apply(x, alpha)

    @staticmethod
    def primitive_function(x: torch.Tensor, alpha):
        return (math.pi / 2 * alpha * x).trangle_() / math.pi + 0.5
