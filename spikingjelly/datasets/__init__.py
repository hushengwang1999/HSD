...
def load_npz_frames(file_name: str) -> np.ndarray:
    ...
    #return np.load(file_name, allow_pickle=True)['arr_0']#修改部分
...
# #def integrate_events_to_frames(events, height, width, frames_num=10, split_by='number', normalization=None):#修改部分
# def integrate_events_by_fixed_frames_number(events: Dict, split_by: str, frames_num: int, H: int, W: int,normalization:str):
#
...
#     # print('correct accumulation by bincount\n', frames)
#     eps = 1e-5
#     frames_max = np.zeros(shape=[frames_num, 2, height, width])
#     frames_norm = np.zeros(shape=[frames_num, 2, height, width])
#     for i in range(frames_num):
...
#         frames1 = frames.reshape(((frames_num, 2, height, width)))
...
#                     frames_max[i] = np.max(frames1, axis=0)
#                     frames_max[i] = frames_max[i] + eps
#                     frames_norm[i] = frames1[i] / frames_max[i]
#             else:
#                 raise NotImplementedError
#
    ...
    def __init__(
        ...
        if data_type == 'event':
            ...
            if frames_number is not None:
                ...
                frames_np_root = os.path.join(root, f'frames_number_{frames_number}_split_by_{split_by}')#修改部分，为了区别之间划分帧的方式，其实就是对数据进行了一个预处理
                #frames_np_root = os.path.join(root,f'frames_number_{frames_number}_split_by_{split_by}_frequency')  # 修改部分，为了区别之间划分帧的方式，其实就是对数据进行了一个预处理
