...
from modules import TCL, MyFloor, ScaledNeuron, StraightThrough,LIFSpike,IFSpike,IFSpike1
...
class tdBatchNorm(nn.Module):
    def __init__(self, out_panel):
        ...
        self.seqbn = layer.SeqToANNContainer(self.bn)
...
def replace_activation_by_neuron1(model):
    for name, module in model._modules.items():
        if hasattr(module,"_modules"):
            model._modules[name] = replace_activation_by_neuron1(module)
        if isActivation(module.__class__.__name__.lower()):
            if hasattr(module, "up"):
                model._modules[name] = IFSpike(scale=module.up.item())
                #model._modules[name] = MultistepIFspike(scale=module.up.item())
                #model._modules[name] = nn.ReLU()
                #model._modules[name] = LIFSpike(scale=module.up.item())
            else:
                model._modules[name] = IFSpike(scale=1.)
                #model._modules[name] = MultistepIFspike(scale=1.)
                #model._modules[name] = nn.ReLU()
                #model._modules[name] = LIFSpike(scale=1.)
    return model

##直接利用ANN，而不是将ANN转化成SNN
def replace_activation_by_neuron2(model):
    for name, module in model._modules.items():
        if hasattr(module,"_modules"):
            model._modules[name] = replace_activation_by_neuron2(module)
        if isActivation(module.__class__.__name__.lower()):
...
def replace_batchnorm_with_tdbatchnorm(model):
    for name, module in model.named_children():
        if isinstance(module, nn.BatchNorm2d):
            tdbatchnorm = tdBatchNorm(module.num_features)
            setattr(model, name, tdbatchnorm)
        else:
            replace_batchnorm_with_tdbatchnorm(module)
